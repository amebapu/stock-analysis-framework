...
- 若`stock-data finance <code> cashflow`返回错误或空数据：
  ...
  3. 评分公式：实际得分/20分
  4. **分值映射**：最终评级时，按比例映射到100分制
     ```
     映射后分数 = (实际得分 / 实际满分) × 100
     例：基本面18/20分 → 映射为 18/20×100 = 90分
     ```
...
**步骤2：分值映射到100分制（当实际满分≠100时）**
```
映射后总分 = (实际总分 / 实际满分) × 100

示例1（现金流未获取）：
  基本面：18/20分，技术面：50/60分，催化剂：12/15分
  实际总分 = 80/95分
  映射后 = 80/95 × 100 = 84.2分 (A级)

示例2（ROE和现金流均未获取）：
  基本面：12/15分，技术面：50/60分，催化剂：12/15分
  实际总分 = 74/90分
  映射后 = 74/90 × 100 = 82.2分 (A级)
```
...
**总分计算（含映射）**：
```
实际满分 = 25分(基本面全有) + 60分 + 15分 = 100分
实际总分 = 15 + 38 + 8 = 61分
映射后分数 = 61/100 × 100 = 61分
评级: C级 (60-69分区间)
```

若现金流未获取示例：
```
实际满分 = 20分(剔除现金流) + 60 + 15 = 95分
实际总分 = 15 + 38 + 8 = 61分
映射后分数 = 61/95 × 100 = 64.2分
评级: C级 (仍为C级，但分数更公平)
```
