...
name: sepa
description: >
  米勒维尼 SEPA 投资分析框架。当用户要求分析某只股票、做投资评分、SEPA评分、
  米勒维尼分析、深度研究、双体系分析、或者明确点名 sepa 时，必须调用此 skill。
  本 skill 负责按框架组织分析流程并输出评分报告，数据获取通过 stock-data skill 完成。
  不要与 stock-data 混淆：stock-data 只负责取数据，sepa 负责分析和评分。
date: "2026-03-10"
type: knowledge
category: "investment"
tags: ["米勒维尼", "SEPA", "股票分析", "中短线", "长线", "深度研究", "100分制", "分值映射", "基本面确定性", "总分汇总", "ETF分类", "stock", "equity", "analysis", "score"]
sources: ["stock-data", "calc_indicators.py", "calc_fundamentals.py", "calc_score.py", "股市魔法师", "彼得林奇", "巴菲特"]
created_by: "AI Assistant"
last_updated: "2026-03-11T0800"
---

# SEPA 投资分析框架 v7.0

> **Skill 短名**: `sepa` — 用户点名或触发后加载  
> **核心方法论**: 马克·米勒维尼 SEPA（中短线）+ 彼得林奇/巴菲特深度研究（长线）  
> **数据源**: `stock-data` skill（腾讯自选股） + `web_search`（深度研究补充）  
> **计算层**: `calc_indicators.py` v1.3.0 + `calc_fundamentals.py` v2.0.0 + `calc_score.py` v1.1.0
...
### 何时调用此 Skill
- 用户说"分析 XXX""评分 XXX""SEPA XXX""米勒维尼 XXX"
- 用户说"深度研究 XXX""双体系分析 XXX"
- 用户直接点名"sepa""调用 sepa"
- 用户要对股票做 100 分制评分、仓位建议、风控检查

### 何时不调用此 Skill
- 只是查行情、看 K 线、看新闻（用 `stock-data`）
- 纯技术指标计算不做评分（用 `stock-data` + Python 脚本）
- 与投资分析无关的任务

### 显式点名优先规则
- 当用户明确说"用 sepa"或"调用 sepa"时，**必须优先调用本 skill，不得跳过**
- 加载后必须按本文档的流程和格式执行，不得自行简化

### 与 stock-data 的职责边界
| Skill | 职责 | 产出 |
|-------|------|------|
| `stock-data` | 获取原始数据（K线、行情、财务、新闻等） | 原始数据 |
| `sepa` | 按米勒维尼框架组织分析、评分、出报告 | 评分报告 |

### 失败兜底
- 如果 `stock-data` 无法获取某项数据，对应评分项标记 `N/A（数据不足）`，不编造
- 美股 K 线不足时使用 qt 涨跌幅统计 + profile 嵌套 K 线作为替代，并标注数据完整性等级

---

## ⚙️ 强制要求（执行红线）

1. **252 根 K 线优先**：技术面计算必须请求 252 根日 K 线
2. **大盘环境必查**：每次分析必须先检查大盘环境（SPY/沪深300），不跳过
3. **禁止估算**：不能"大概""应该""可能"，无数据标"未获取"
4. **禁止编造**：没有的数据不能说"估计是 XXX"
5. **禁止心算**：技术指标必须通过 Python 脚本计算，禁止大模型心算
6. **缺项映射**：无法获取的评分项按缺项映射表处理，不跳过
7. **双体系默认输出**：除非用户明确说"不需要深度研究"，否则默认输出 SEPA + 深度研究
8. **有效满分 + 置信等级**：报告必须标注有效满分和置信等级
...
| 总分 | 等级 | 建议 | 仓位 | 说明 |
|------|------|------|------|------|
| ≥85 | A+ | **强力买入候选** | **15%~20%** | SEPA完美+基本面优秀+有催化剂 |
| ≥70 | A | **买入候选** | **10%~15%** | 满足硬门槛，技术面良好 |
| ≥55 | B | **观察名单** | **5%~10%** | 有瑕疵，需严格止损 |
| ≥40 | C | **谨慎观望** | **0%~5%** | 某项硬门槛未达标或SEPA<4项 |
| <40 | D | **排除** | **0%** | 触碰红线或多项不合格 |
